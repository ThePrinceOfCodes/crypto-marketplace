...
import { useGetUsersBalance } from "api/hooks";
import {
  inViewport,
...
function TDUsersBalance() {
  ...
  const { data, isLoading, isFetching, fetchNextPage } = useGetUsersBalance({
    ...
    () => [
      {
        ...sharedColDef,
        field: "email",
        headerName: `${text("token_details_user_email")}`,
        minWidth: 100,
        flex: 1,
        width: 100,
      ...
      },
      {
        ...
        renderCell: ({ row }) => (
          <div className="flex gap-1 w-full">
            ...
            <Tooltip title={row?.tokenName}>
              <span className="truncate ellipsis">{row?.tokenName}</span>
            </Tooltip>
          </div>
        ),
      },
  ...
  } = useGridColumnChange("tdUsersBalanceColumnState");
  ...
  return (
    <div className="flex h-full w-full flex-col gap-3">
      ...
      >
        <DataGridPro
          rows={userBalanceIndex || []}
          rowCount={userBalanceIndex?.length || 0}
